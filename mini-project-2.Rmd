...
```{r}
#I want to tidy the first 8 variables of the data set by splitting the "bike_parking" variable into 3 variables, one for outdoor bike parking, one for indoor bike parking, and one for no bike parking. 
...
```{r}
percent_accessibility_order <- tidy_accessibility_data %>% 
  mutate(barrier_free_accessibilty_entr = fct_reorder(barrier_free_accessibilty_entr, id))
percent_accessibility_order
```
...
```{r}
#grouping buildings with no heating type specified into an "other" category 
...
saveRDS(lin_reg_percent_accessible, file = "lin_reg_percent_accessible.rds", ascii = FALSE, version = NULL, compress = TRUE, here::here("output", "lin_reg_percent_accessible.rds"))
